...
export default function PreferencesScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      for (const p of prefsData) pState[p.item_name] = p.preference_value;
      setPrefs(pState);
      ...
      >
        ...
        {filteredMeals.length === 0 ? (
          ...
          filteredMeals.map(meal => {
            const pref = prefs[meal.name] || 0;
            return (
              <TouchableOpacity key={meal.id} testID={`pref-item-${meal.id}`} style={[styles.prefCard, pref === 1 && styles.prefCardLiked, pref === -1 && styles.prefCardDisliked]} onPress={() => togglePref(meal.name)} activeOpacity={0.7}>
