  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      for (const r of ratingsData) {
        rState[r.item_name] = { servings: r.servings || 0, rating: r.rating || 0 };
      }
      setRatings(rState);
      ...
      >
        ...
        {filteredMeals.length === 0 ? (
          ...
          filteredMeals.map(meal => {
            const r = ratings[meal.name] || { servings: 0, rating: 0 };
            return (
                ...
                <View style={styles.controlsRow}>
                  <View style={styles.servingsBlock}>
                    ...
                    <View style={styles.servingsRow}>
                      <TouchableOpacity testID={`servings-minus-${meal.id}`} style={styles.servBtn} onPress={() => updateRating(meal.name, 'servings', r.servings - 1)}>
                        <Ionicons name="remove" size={18} color={COLORS.textPrimary} />
                      </TouchableOpacity>
                      <Text style={styles.servCount}>{r.servings}</Text>
                      <TouchableOpacity testID={`servings-plus-${meal.id}`} style={styles.servBtn} onPress={() => updateRating(meal.name, 'servings', r.servings + 1)}>
                  ...
                  <View style={styles.starsBlock}>
                    ...
                    <View style={styles.starsRow}>
                      {[1, 2, 3, 4, 5].map(star => (
                        <TouchableOpacity key={star} testID={`star-${meal.id}-${star}`} onPress={() => updateRating(meal.name, 'rating', star)}>
